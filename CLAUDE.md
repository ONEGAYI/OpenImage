...
│   ├── src/
│   │   ├── components/             # UI 组件
│   │   │   ├── ChatPanel/          # AI 聊天面板（输入框侧栏，7 个子组件）
│   │   │   │   ├── index.tsx       # 聊天面板容器
│   │   │   │   ├── ChatMessage.tsx # 消息气泡（支持 AiBlock 渲染）
...
│   │   │   ├── RatioSelector.tsx   # 比例/尺寸选择 Popover
...
3. SSE 流式返回 → 前端实时渲染消息 + AiBlock 结构化内容
4. 消息持久化到 `llm_messages` 表
