...
│   │   │   ├── inpaint.py          # POST /api/inpaint — 局部重绘（mask + 原图 → SSE，自动锁定源图比例）
│   │   │   ├── sessions.py         # 会话 CRUD
│   │   │   └── settings.py         # API 设置（key/url/mode/model）+ 版本信息
│   │   ├── core/                   # 业务核心层
│   │   │   ├── client.py           # OpenAI API 客户端（三种模式 + inpainting 路由 + Images edits 多步迭代）
...
└── docs/superpowers/               # 设计文档与实施计划
    ├── specs/                      # 功能规格说明（6 篇：inpainting、version-info、redesign-migration、openimage、ratio-size-control、i18n-design）
    └── plans/                      # 实施计划（6 篇，与 specs 对应）
```
