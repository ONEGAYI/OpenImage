...
│   │   │   ├── sessions.py         # 图片生成会话 CRUD + Fork 分支端点
...
1. 用户在 DetailPanel 点击 Fork 按钮 → `POST /api/sessions/{id}/fork`（携带 `image_id`）
2. `SessionManager.fork()` 查询目标图片及之前所有图片记录，创建新会话（自动命名 `原会话名 (Fork #N)`）
3. `Storage.copy_session_images()` 物理拷贝图片文件到新会话目录（源文件缺失时抛出 `FileNotFoundError`）
4. 新会话继承 `head_response_id`，后续生成基于 Fork 点独立迭代
