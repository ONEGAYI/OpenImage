...
├── scripts/
│   ├── build.py                    # 一键构建：PyInstaller → sidecar 部署 → Tauri 打包
│   ├── bump.mjs                    # 版本号管理脚本
...
2. 传递 `--base-dir` 指向 `%APPDATA%/OpenImage`（系统标准应用数据目录）
3. 健康检查轮询 `/api/settings`（最多 30s），就绪后 emit `backend-ready`
4. 前端 `App.tsx` 的 loading gate 等待后端就绪后再渲染主 UI
...
- **数据目录**: 开发环境用项目根目录，打包后用 `%APPDATA%/OpenImage`
...
- CORS 白名单包含 `tauri://localhost`、`https://tauri.localhost`、`http://localhost:1420`
