...
│   │   │   ├── inpaint.py          # POST /api/inpaint — 局部重绘（mask + 原图 + 可选参考图 → SSE，自动锁定源图比例）
│   │   │   ├── sessions.py         # 会话 CRUD
│   │   │   └── settings.py         # API 设置（key/url/mode/model）+ 版本信息
│   │   ├── core/                   # 业务核心层
│   │   │   ├── client.py           # OpenAI API 客户端（三种模式 + inpainting 路由 + 参考图 + Images edits 多步迭代）
...
│   │   │   ├── RatioSelector.tsx   # 比例/尺寸选择 Popover（工具栏触发按钮 + 向上弹出选单）
│   │   │   ├── InputArea.tsx       # 输入区 + 图片附件 + 参考图传递（附件 hover 显示 Inpaint 编辑图标）
...
│   │   ├── services/
│   │   │   └── api.ts              # HTTP + SSE 通信层（动态端口 + initBaseUrl）
│   │   ├── utils/
...
└── docs/superpowers/               # 设计文档与实施计划
    ├── specs/                      # 功能规格说明（8 篇：inpainting、version-info、redesign-migration、openimage、ratio-size-control、i18n-design、dynamic-port、mask-reference-attachments）
    └── plans/                      # 实施计划（8 篇，与 specs 对应）
```
...
三个 Zustand store，无路由：
- `sessionStore` — 会话列表、图片列表、**多选状态**（`selectedImageIds: string[]`，`toggleImageSelect` 切换，`clearSelection` 清空）
- `generationStore` — 生成流程（**会话级隔离**：`sessionGenerations` Map，每会话独立 isGenerating/partialImage/abortController）、附件、中断。完成时动态 import `sessionStore` 避免循环依赖，并行调用 `fetchSessions()` + `selectSession()` 刷新
